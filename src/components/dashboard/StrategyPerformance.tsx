...
import { type StrategyPerformanceRow } from "@/lib/mock-data";
        ...
        <table className="min-w-full border-separate border-spacing-y-2">
          ...
          <tbody>
            {rows.map((row) => {
              ...
              const factorWidth = `${Math.min((row.profitFactor / 2.5) * 100, 100)}%`;

              return (
                <tr key={row.id} className="bg-transparent">
                  <td className="rounded-l-[18px] bg-[rgba(250,250,255,0.88)] px-3 py-4">
                    <div className="font-semibold text-slate-900">
                      {copy.strategies[row.id]}
                    </div>
                  </td>
                  <td className="bg-[rgba(250,250,255,0.88)] px-3 py-4 font-semibold text-emerald-600">
